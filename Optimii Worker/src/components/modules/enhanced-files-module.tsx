...
import { Upload, FileText, Download, Trash2, CheckCircle2, XCircle, Clock, FolderOpen, X, Eye, Grid, List, User, MessageSquare, AlertCircle } from "lucide-react";
...
import { RoundsSelector } from "@/components/modules/rounds-selector";
import { CommentsSection } from "@/components/modules/comments-section";
import { FilePreview, FileThumbnail, getFileType } from "@/components/files";
import { CostSection } from "@/components/costs";
import type { Stage, User as UserType, Contact, File as FileType } from "@/lib/db/schema";
...
interface EnhancedFilesModuleProps {
  stage: Stage;
  projectId: string;
  phaseId: string;
...
export function EnhancedFilesModule({ stage, projectId, phaseId, currentUserId }: EnhancedFilesModuleProps) {
  ...
  const handleApprovalResponse = async (notes?: string) => {
    ...
    try {
      if (approvalResponseType === "approve") {
        const result = await approveApproval(selectedApproval.id, currentUser.id, notes);
        if (result) {
        ...
        await rejectApproval(selectedApproval.id, currentUser.id, notes);
      }
            ...
            {files.length > 0 && (
              <div className="flex items-center border rounded-md">
                <Button
                  variant="ghost"
                  size="icon"
                  className={`h-7 w-7 rounded-r-none ${viewMode === "list" ? "bg-muted" : ""}`}
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className={`h-7 w-7 rounded-l-none ${viewMode === "grid" ? "bg-muted" : ""}`}
